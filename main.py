...
# L¥y GitHub username tñ Ùng
github_user = os.getenv("GITHUB_ACTOR", "vonga2221983")
URL = f"https://adnade.net/ptp/?user=tuanzobe&subid={github_user}"
print("URL ang mß:", URL)

TOTAL_RUN_HOURS = 6   # m×i job tÑi a 6 giÝ
OPEN_MINUTES = 60      # mß trang 60 phút
DELAY_MINUTES = 3      # nghÉ 35 phút giïa các vòng
...
def run_loop():
    ...
    with sync_playwright() as p:
        while time.time() < end_time:
            ...
            print(f"=I Mß trang: {URL}")
            page.goto(URL, wait_until="load")
            print(f"ó Giï trang mß ~{OPEN_MINUTES} phút...")
            human_actions(page)
            time.sleep(OPEN_MINUTES * 60)

            browser.close()
            rest = random.randint(DELAY_MINUTES, DELAY_MINUTES + 2)
            print(f"ò NghÉ {rest} phút tr°Ûc vòng ti¿p theo...")
            time.sleep(rest * 60)

    print(" Hoàn t¥t 6 giÝ ch¡y  Workflow s½ tñ ch¡y l¡i n¿u không có job ci ang ch¡y.")
